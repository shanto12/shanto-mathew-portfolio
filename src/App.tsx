...
import { achievements, agenticTools, principles, profile, skillGroups } from './data/profile'
...
const contactIntents = ['Security automation', 'Agent workflow', 'AI product', 'Voice systems'] as const
...
function Hero() {
  return <section className="hero section-shell" id="home"><div className="hero-copy"><div className="hero-kicker"><StatusPill>OPEN TO THE RIGHT PROBLEM</StatusPill><span className="mono">AI × SECURITY × PRODUCT</span></div><p className="eyebrow">Forward Deployed AI Engineer</p><h1>I make complex systems<br /><span>feel inevitable.</span></h1><p className="hero-summary">I&apos;m Shanto Mathew — a Forward Deployed AI Engineer building production GenAI for enterprise banking and AI-enhanced security operations.</p><div className="hero-profile"><div className="hero-profile-frame"><img className="hero-profile-photo" src="/shanto-mathew-headshot.jpeg" alt="Shanto Mathew smiling in a suit" /><span className="hero-profile-badge"><span className="status-pulse" /> profile online</span></div><div className="hero-profile-copy"><span className="mono">SHANTO MATHEW / FDE</span><strong>Human context + machine speed.</strong><span>12+ years IT · 2+ years production AI</span></div></div><div className="hero-actions"><a className="button button--primary" href="#work">Explore the work <ArrowDownRight size={17} /></a><a className="button button--ghost" href="#contact">Start a conversation <Mail size={17} /></a></div><div className="hero-proof-row"><span><MapPin size={15} /> Dallas–Fort Worth · remote-friendly</span><span><CheckCircle2 size={15} /> public-safe by design</span></div></div><HeroWorkbench /></section>
}
...
function AgentRelay() {
  ...
  const tool = agenticTools[activeTool]
  return <section className="section-shell relay-section" id="toolchain" aria-labelledby="relay-heading"><div className="section-intro section-intro--split"><div><p className="eyebrow">The agent relay</p><h2 id="relay-heading">Many minds.<br /><span>One shipping loop.</span></h2></div><p>Different tools are useful at different moments. I keep the handoff explicit: context in, artifact out, human judgment at the boundary.</p></div><div className="relay-layout"><div className="relay-rail" role="tablist" aria-label="Agentic tools in the loop">{agenticTools.map((item, index) => <button className={activeTool === index ? `relay-tab relay-tab--active relay-tab--${item.color}` : 'relay-tab'} type="button" role="tab" aria-selected={activeTool === index} key={item.name} onClick={() => setActiveTool(index)}><span className="relay-index">0{index + 1}</span><span><strong>{item.name}</strong><small>{item.mode}</small></span><ChevronRight size={16} /></button>)}</div><div className={`relay-detail relay-detail--${tool.color}`}><div className="relay-detail-top"><span className="mono">TOOL IN THE LOOP / 0{activeTool + 1}</span><span className="relay-artifact"><CheckCircle2 size={13} /> {tool.artifact}</span></div><div className="relay-node-field" aria-hidden="true"><div className="relay-node relay-node--left">context</div><div className="relay-node relay-node--center">{tool.name}</div><div className="relay-node relay-node--right">human<br />signal</div><div className="relay-connector relay-connector--one" /><div className="relay-connector relay-connector--two" /></div><h3>{tool.name}</h3><p>{tool.copy}</p><div className="relay-foot"><span className="status-pulse" /> explicit handoff / reversible move</div></div></div></section>
}
...
function Contact() {
  ...
  const chooseIntent = (intent: string) => setPayload((current) => ({ ...current, company: intent, message: current.message || `I want to explore a better ${intent.toLowerCase().replace(/ workflow$/, '')} workflow.` }))
...
export default function App() {
  return <><a className="skip-link" href="#main-content">Skip to content</a><ScrollProgress /><Header /><main id="main-content"><Hero /><Marquee /><Impact /><AgentRelay /><Skills /><DemoGallery /><Method /><Contact /></main><Footer /></>
}
